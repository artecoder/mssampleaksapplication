...
namespace SpeechApp
{
    public class AudioCapturePermissions
    {
        ...
        public async static Task<bool> RequestMicrophonePermission(int microphoneNumber)
        {
            try
            {
                ...
                {
                    var dialog = new MessageDialog("Debe seleccionar un micrófono para realizar la grabación");
                    await dialog.ShowAsync();
                    throw new Exception();
                }

                DeviceInformation microphoneDevice = allAudioCaptureDevices[microphoneNumber];

                //allAudioCaptureDevices.FirstOrDefault(x => x.EnclosureLocation != null &&
                //    x.EnclosureLocation.Panel == Windows.Devices.Enumeration.Panel.Back);

                //microphoneDevice = microphoneDevice ?? allAudioCaptureDevices[1];

                if (microphoneDevice == null)
                {
                    var dialog = new MessageDialog("No se ha encontrado un microfono en su sistema.");
                    await dialog.ShowAsync();
                }

                // Request access to the microphone only, to limit the number of capabilities we need
                // to request in the package manifest.
                MediaCaptureInitializationSettings settings = new MediaCaptureInitializationSettings { AudioDeviceId = microphoneDevice.Id };
            ...
            catch (UnauthorizedAccessException)
            {
                ...
                var dialog = new MessageDialog("No se ha podido acceder al microfono instalado.");
                await dialog.ShowAsync();
                return false;
            }
